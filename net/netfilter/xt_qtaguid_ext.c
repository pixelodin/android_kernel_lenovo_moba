			    ...
			    const struct net_device **el_dev)
{
	...
	if (skb->dev && *el_dev != skb->dev) {
		pr_debug("qtaguid[%d]: skb->dev=%p %s vs par->%s=%p %s\n",
		    ...
		    struct xt_action_param *par)
{
	...
	if (dir == IFS_RX && need_fix_uid(net_dev->name, proto)) {
		get_addr_and_port(skb, dir, xt_family(par), ap);
		spin_lock_bh(&recent_owner_lock);
		entry = __recent_owner_lookup(ap, xt_family(par));
		if (entry != NULL)
			uid = entry->uid;
		else
			uid = 0;
		    ...
		    int debug_os_data)
{
	...
	spin_lock_bh(&recent_owner_lock);
	get_addr_and_port(skb, IFS_TX, xt_family(par), ap);
	entry = __recent_owner_lookup(ap, xt_family(par));
	if (entry != NULL)
		entry->uid = uid;
	...
	if (entry != NULL) {
		memcpy(&entry->addr, &ap[1], 4 * ADDR_SIZE(xt_family(par)));
		entry->sport = SPORT(ap);
		entry->dport = DPORT(ap);
	}
end:
