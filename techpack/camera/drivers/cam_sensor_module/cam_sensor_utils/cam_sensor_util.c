		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			if((power_setting->seq_type == SENSOR_CUSTOM_GPIO1) && (soc_info->index == 1)) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (power_setting->seq_val != INVALID_VREG)
			{
				...
				if((power_setting->seq_type == SENSOR_VIO) && ((soc_info->index == 2) || (soc_info->index == 0))) {
					rc = cam_power_ldo_control(soc_info->index, true);
					if (rc < 0)
						CAM_ERR(CAM_SENSOR, "wl set cam%d vol failed: rc: %d", soc_info->index, rc);
				}
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			if (!gpio_num_info->valid[pd->seq_type])
				continue;
#ifdef CONFIG_PRODUCT_MOBA
			if((pd->seq_type == SENSOR_CUSTOM_GPIO1) && (soc_info->index == 1)) {
		...
		case SENSOR_CUSTOM_REG1:
		case SENSOR_CUSTOM_REG2:
			if((pd->seq_type == SENSOR_VIO) && ((soc_info->index == 2) || (soc_info->index == 0))) {
				 ret = cam_power_ldo_control(soc_info->index, false);
				 if (ret < 0)
					 CAM_ERR(CAM_SENSOR, "wl set cam%d vol failed: rc: %d", soc_info->index, ret);
			}
