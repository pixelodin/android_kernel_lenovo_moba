...
extern int mipi_dsi_dcs_get_display_osc(struct mipi_dsi_device *dsi);
extern int mipi_dsi_dcs_set_display_osc(struct mipi_dsi_device *dsi);
extern int mipi_dsi_dcs_get_display_gamma(struct mipi_dsi_device *dsi);
extern int mipi_dsi_dcs_get_display_gamma_c(struct mipi_dsi_device *dsi, int c);
extern int mipi_dsi_dcs_get_display_gamma_normal(struct mipi_dsi_device *dsi);
u8 osc_val[28];
u8 gamma_c7[154];
u8 gamma_c8[154];
u8 gamma_c9[154];
u8 gamma_ca[154];
u8 gamma_cb[154];
u8 gamma_c7_normal[154];
u8 gamma_c8_normal[154];
u8 gamma_c9_normal[154];
u8 gamma_ca_normal[154];
u8 gamma_cb_normal[154];
int first_read_osc_gamma = 1;
int first_check_gamma_error = 0;

void dsi_display_check_gamma_status(void)
{
	int index = 0;
	int index_temp = 0;
	int error_index = 0;
	...
	for (index = 0; index < 144; index++) {
		...
		if (gamma_c7[index_temp] == 0xff)
			error_index++;

		index_temp++;

		if((gamma_c7[index_temp] != 0xff) && (error_index < 6))
			error_index = 0;
	}

	if (error_index >= 6) {
		printk("drm dsi C7 gamma check error! error_index %d\n", error_index);
		first_check_gamma_error = 1;
		return;
	}
        ...
        for (index = 0; index < 144; index++) {
                ...
                if (gamma_c8[index_temp] == 0xff)
                        error_index++;

                index_temp++;

                if((gamma_c8[index_temp] != 0xff) && (error_index < 6))
                        error_index = 0;
        }

        if (error_index >= 6) {
                printk("drm dsi C8 gamma check error! error_index %d\n", error_index);
		first_check_gamma_error = 1;
                return;
        }
        ...
        for (index = 0; index < 144; index++) {
                ...
                if (gamma_c9[index_temp] == 0xff)
                        error_index++;

                index_temp++;

                if((gamma_c9[index_temp] != 0xff) && (error_index < 6))
                        error_index = 0;
        }

        if (error_index >= 6) {
                printk("drm dsi C9 gamma check error! error_index %d\n", error_index);
                first_check_gamma_error = 1;
                return;
	}
        ...
        for (index = 0; index < 144; index++) {
                ...
                if (gamma_ca[index_temp] == 0xff)
                        error_index++;

                index_temp++;

                if((gamma_ca[index_temp] != 0xff) && (error_index < 6))
                        error_index = 0;
        }

        if (error_index >= 6) {
                printk("drm dsi CA gamma check error! error_index %d\n", error_index);
		first_check_gamma_error = 1;
                return;
	}
        ...
        for (index = 0; index < 144; index++) {
                ...
                if (gamma_cb[index_temp] == 0xff)
                        error_index++;

                index_temp++;

                if((gamma_cb[index_temp] != 0xff) && (error_index < 6))
                        error_index = 0;
        }

        if (error_index >= 6) {
                printk("drm dsi CB gamma check error! error_index %d\n", error_index);
		first_check_gamma_error = 1;
                return;
	}
...
int dsi_display_read_osc_status(struct dsi_display_ctrl *ctrl,
		struct dsi_panel *panel)
{
	int rc = 0;
	struct dsi_cmd_desc cmds;
	u8 data[] = {06, 01, 00, 01, 00, 00, 01, 0xE4};
	u32 flags = 0;
	u8 *payload;
	int size, j;
	...
	mipi_dsi_dcs_get_display_osc(dsi);
	...
	printk("drm dsi osc E4 val = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
					osc_val[0], osc_val[1], osc_val[2], osc_val[3], osc_val[4], osc_val[5], osc_val[6], osc_val[7], osc_val[8], osc_val[9],
					osc_val[10], osc_val[11], osc_val[12], osc_val[13], osc_val[14], osc_val[15], osc_val[16], osc_val[17], osc_val[18], osc_val[19]);
error:
	return 0;
}

int dsi_display_read_gamma_status(struct dsi_display_ctrl *ctrl,
		struct dsi_panel *panel)
{
	int rc = 0;
	struct dsi_cmd_desc cmds;
	u8 data[] = {06, 01, 00, 01, 00, 00, 01, 0xF4};
	u32 flags = 0;
	u8 *payload;
	int size, j;
	struct mipi_dsi_device *dsi;
	...
	dsi = &panel->mipi_device;
	mipi_dsi_dcs_get_display_gamma(dsi);
	mipi_dsi_dcs_get_display_gamma_c(dsi, 7);
	...
	for (j = 0; j < 7; j++)
	printk("drm dsi gamma C7 val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
					gamma_c7[j*20+0], gamma_c7[j*20+1], gamma_c7[j*20+2], gamma_c7[j*20+3], gamma_c7[j*20+4], gamma_c7[j*20+5], gamma_c7[j*20+6], gamma_c7[j*20+7], gamma_c7[j*20+8], gamma_c7[j*20+9],
					gamma_c7[j*20+10], gamma_c7[j*20+11], gamma_c7[j*20+12], gamma_c7[j*20+13], gamma_c7[j*20+14], gamma_c7[j*20+15], gamma_c7[j*20+16], gamma_c7[j*20+17], gamma_c7[j*20+18], gamma_c7[j*20+19]);
	printk("drm dsi gamma C7 val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c7[140], gamma_c7[141], gamma_c7[142], gamma_c7[143]);

	// C8 GAMMA
	mipi_dsi_dcs_get_display_gamma_c(dsi, 8);
	msleep(5);
        cmds.msg.rx_buf = &gamma_c8[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma C8 val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_c8[j*20+0], gamma_c8[j*20+1], gamma_c8[j*20+2], gamma_c8[j*20+3], gamma_c8[j*20+4], gamma_c8[j*20+5], gamma_c8[j*20+6], gamma_c8[j*20+7], gamma_c8[j*20+8], gamma_c8[j*20+9],
                                        gamma_c8[j*20+10], gamma_c8[j*20+11], gamma_c8[j*20+12], gamma_c8[j*20+13], gamma_c8[j*20+14], gamma_c8[j*20+15], gamma_c8[j*20+16], gamma_c8[j*20+17], gamma_c8[j*20+18], gamma_c8[j*20+19]);
        printk("drm dsi gamma C8 val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c8[140], gamma_c8[141], gamma_c8[142], gamma_c8[143]);

        // C9 GAMMA
        mipi_dsi_dcs_get_display_gamma_c(dsi, 9);
	msleep(5);
        cmds.msg.rx_buf = &gamma_c9[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma C9 val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_c9[j*20+0], gamma_c9[j*20+1], gamma_c9[j*20+2], gamma_c9[j*20+3], gamma_c9[j*20+4], gamma_c9[j*20+5], gamma_c9[j*20+6], gamma_c9[j*20+7], gamma_c9[j*20+8], gamma_c9[j*20+9],
                                        gamma_c9[j*20+10], gamma_c9[j*20+11], gamma_c9[j*20+12], gamma_c9[j*20+13], gamma_c9[j*20+14], gamma_c9[j*20+15], gamma_c9[j*20+16], gamma_c9[j*20+17], gamma_c9[j*20+18], gamma_c9[j*20+19]);
        printk("drm dsi gamma C9 val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c9[140], gamma_c9[141], gamma_c9[142], gamma_c9[143]);

        // CA GAMMA
        mipi_dsi_dcs_get_display_gamma_c(dsi, 10);
	msleep(5);
        cmds.msg.rx_buf = &gamma_ca[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma CA val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_ca[j*20+0], gamma_ca[j*20+1], gamma_ca[j*20+2], gamma_ca[j*20+3], gamma_ca[j*20+4], gamma_ca[j*20+5], gamma_ca[j*20+6], gamma_ca[j*20+7], gamma_ca[j*20+8], gamma_ca[j*20+9],
                                        gamma_ca[j*20+10], gamma_ca[j*20+11], gamma_ca[j*20+12], gamma_ca[j*20+13], gamma_ca[j*20+14], gamma_ca[j*20+15], gamma_ca[j*20+16], gamma_ca[j*20+17], gamma_ca[j*20+18], gamma_ca[j*20+19]);
        printk("drm dsi gamma CA val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_ca[140], gamma_ca[141], gamma_ca[142], gamma_ca[143]);

        // CB GAMMA
        mipi_dsi_dcs_get_display_gamma_c(dsi, 11);
	msleep(5);
        cmds.msg.rx_buf = &gamma_cb[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma CB val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_cb[j*20+0], gamma_cb[j*20+1], gamma_cb[j*20+2], gamma_cb[j*20+3], gamma_cb[j*20+4], gamma_cb[j*20+5], gamma_cb[j*20+6], gamma_cb[j*20+7], gamma_cb[j*20+8], gamma_cb[j*20+9],
                                        gamma_cb[j*20+10], gamma_cb[j*20+11], gamma_cb[j*20+12], gamma_cb[j*20+13], gamma_cb[j*20+14], gamma_cb[j*20+15], gamma_cb[j*20+16], gamma_cb[j*20+17], gamma_cb[j*20+18], gamma_cb[j*20+19]);
        printk("drm dsi gamma CB val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_cb[140], gamma_cb[141], gamma_cb[142], gamma_cb[143]);
error:
	return 0;
}

int dsi_display_read_gamma_normal_status(struct dsi_display_ctrl *ctrl,
		struct dsi_panel *panel)
{
	int rc = 0;
	struct dsi_cmd_desc cmds;
	u8 data[] = {06, 01, 00, 01, 00, 00, 01, 0xC7};
	u32 flags = 0;
	u8 *payload;
	int size, j;
	struct mipi_dsi_device *dsi;
	...
	dsi = &panel->mipi_device;
	mipi_dsi_dcs_get_display_gamma_normal(dsi);
	...
	// C7 GAMMA
	cmds.msg.rx_buf = &gamma_c7_normal[0];
	cmds.msg.rx_len = 144;
	rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
	if (rc <= 0) {
		pr_err("rx cmd transfer failed rc=%d\n", rc);
	...
	for (j = 0; j < 7; j++)
	printk("drm dsi gamma C7 normal val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
					gamma_c7_normal[j*20+0], gamma_c7_normal[j*20+1], gamma_c7_normal[j*20+2], gamma_c7_normal[j*20+3], gamma_c7_normal[j*20+4], gamma_c7_normal[j*20+5], gamma_c7_normal[j*20+6], gamma_c7_normal[j*20+7], gamma_c7_normal[j*20+8], gamma_c7_normal[j*20+9],
					gamma_c7_normal[j*20+10], gamma_c7_normal[j*20+11], gamma_c7_normal[j*20+12], gamma_c7_normal[j*20+13], gamma_c7_normal[j*20+14], gamma_c7_normal[j*20+15], gamma_c7_normal[j*20+16], gamma_c7_normal[j*20+17], gamma_c7_normal[j*20+18], gamma_c7_normal[j*20+19]);
	printk("drm dsi gamma C7 normal val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c7_normal[140], gamma_c7_normal[141], gamma_c7_normal[142], gamma_c7_normal[143]);
	...
	cmds.msg.tx_buf = payload;
        ...
        cmds.msg.rx_buf = &gamma_c8_normal[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma C8 normal val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_c8_normal[j*20+0], gamma_c8_normal[j*20+1], gamma_c8_normal[j*20+2], gamma_c8_normal[j*20+3], gamma_c8_normal[j*20+4], gamma_c8_normal[j*20+5], gamma_c8_normal[j*20+6], gamma_c8_normal[j*20+7], gamma_c8_normal[j*20+8], gamma_c8_normal[j*20+9],
                                        gamma_c8_normal[j*20+10], gamma_c8_normal[j*20+11], gamma_c8_normal[j*20+12], gamma_c8_normal[j*20+13], gamma_c8_normal[j*20+14], gamma_c8_normal[j*20+15], gamma_c8_normal[j*20+16], gamma_c8_normal[j*20+17], gamma_c8_normal[j*20+18], gamma_c8_normal[j*20+19]);
        printk("drm dsi gamma C8 normal val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c8_normal[140], gamma_c8_normal[141], gamma_c8_normal[142], gamma_c8_normal[143]);
	...
	cmds.msg.tx_buf = payload;
        ...
        cmds.msg.rx_buf = &gamma_c9_normal[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma C9 normal val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_c9_normal[j*20+0], gamma_c9_normal[j*20+1], gamma_c9_normal[j*20+2], gamma_c9_normal[j*20+3], gamma_c9_normal[j*20+4], gamma_c9_normal[j*20+5], gamma_c9_normal[j*20+6], gamma_c9_normal[j*20+7], gamma_c9_normal[j*20+8], gamma_c9_normal[j*20+9],
                                        gamma_c9_normal[j*20+10], gamma_c9_normal[j*20+11], gamma_c9_normal[j*20+12], gamma_c9_normal[j*20+13], gamma_c9_normal[j*20+14], gamma_c9_normal[j*20+15], gamma_c9_normal[j*20+16], gamma_c9_normal[j*20+17], gamma_c9_normal[j*20+18], gamma_c9_normal[j*20+19]);
        printk("drm dsi gamma C9 normal val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_c9_normal[140], gamma_c9_normal[141], gamma_c9_normal[142], gamma_c9_normal[143]);
	...
	cmds.msg.tx_buf = payload;
        ...
        cmds.msg.rx_buf = &gamma_ca_normal[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma CA normal val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_ca_normal[j*20+0], gamma_ca_normal[j*20+1], gamma_ca_normal[j*20+2], gamma_ca_normal[j*20+3], gamma_ca_normal[j*20+4], gamma_ca_normal[j*20+5], gamma_ca_normal[j*20+6], gamma_ca_normal[j*20+7], gamma_ca_normal[j*20+8], gamma_ca_normal[j*20+9],
                                        gamma_ca_normal[j*20+10], gamma_ca_normal[j*20+11], gamma_ca_normal[j*20+12], gamma_ca_normal[j*20+13], gamma_ca_normal[j*20+14], gamma_ca_normal[j*20+15], gamma_ca_normal[j*20+16], gamma_ca_normal[j*20+17], gamma_ca_normal[j*20+18], gamma_ca_normal[j*20+19]);
        printk("drm dsi gamma CA normal val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_ca_normal[140], gamma_ca_normal[141], gamma_ca_normal[142], gamma_ca_normal[143]);
	...
	cmds.msg.tx_buf = payload;
        ...
        cmds.msg.rx_buf = &gamma_cb_normal[0];
        cmds.msg.rx_len = 144;
        rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmds.msg, &flags);
        if (rc <= 0) {
                pr_err("rx cmd transfer failed rc=%d\n", rc);
        ...
        for (j = 0; j < 7; j++)
        printk("drm dsi gamma CB normal val %d = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", j,
                                        gamma_cb_normal[j*20+0], gamma_cb_normal[j*20+1], gamma_cb_normal[j*20+2], gamma_cb_normal[j*20+3], gamma_cb_normal[j*20+4], gamma_cb_normal[j*20+5], gamma_cb_normal[j*20+6], gamma_cb_normal[j*20+7], gamma_cb_normal[j*20+8], gamma_cb_normal[j*20+9],
                                        gamma_cb_normal[j*20+10], gamma_cb_normal[j*20+11], gamma_cb_normal[j*20+12], gamma_cb_normal[j*20+13], gamma_cb_normal[j*20+14], gamma_cb_normal[j*20+15], gamma_cb_normal[j*20+16], gamma_cb_normal[j*20+17], gamma_cb_normal[j*20+18], gamma_cb_normal[j*20+19]);
        printk("drm dsi gamma CB normal val 7 = 0x%x, 0x%x, 0x%x, 0x%x\n", gamma_cb_normal[140], gamma_cb_normal[141], gamma_cb_normal[142], gamma_cb_normal[143]);
error:
	return 0;
}
#endif
...
#ifdef CONFIG_PRODUCT_MOBA
	if (first_read_osc_gamma == 1) {
		dsi_display_read_osc_status(ctrl, panel);
		dsi_display_read_gamma_normal_status(ctrl, panel);
		dsi_display_read_gamma_status(ctrl, panel);
		dsi_display_check_gamma_status();
		first_read_osc_gamma = 0;
		mipi_dsi_dcs_set_display_osc(&panel->mipi_device);
	}
#endif
					...
					unsigned int display_type)
{
	...
	if (strstr(boot_str, "no_panel")) {
		dsi_no_panel = 1;
		DSI_ERR("can't found panel\n");
	} else
		DSI_ERR("found panel\n");
		...
		void *data)
{
	...
	int i, rc = 0;
        ...
        if (dsi_display_no_panel()) {
                DSI_ERR("NO PANEL, no enter dsi_display_bind function\n");
                return -ENODEV;
        }
