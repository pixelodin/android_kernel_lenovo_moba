...
extern int first_read_osc_gamma;
extern int first_check_gamma_error;
extern int mipi_dsi_dcs_set_display_osc(struct mipi_dsi_device *dsi);
extern int mipi_dsi_dcs_set_display_gamma(struct mipi_dsi_device *dsi);
extern int mipi_dsi_dcs_set_display_gamma_normal(struct mipi_dsi_device *dsi);
#endif
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	if ((old_fps <= 90) && (now_fps <= 90)) {
		printk("drm dsi_panel_switch fps <= 90 +++++++++++++\n");
...
*/
		...
		if (((now_fps == 120) || (now_fps == 144)) &&(first_read_osc_gamma == 0) && (first_check_gamma_error == 0)) {
			//mipi_dsi_dcs_set_display_osc(&panel->mipi_device);
			mipi_dsi_dcs_set_display_gamma(&panel->mipi_device);
		}

		if (((old_fps == 120) || (old_fps == 144)) && (first_read_osc_gamma == 0))
			mipi_dsi_dcs_set_display_gamma_normal(&panel->mipi_device);
	}
