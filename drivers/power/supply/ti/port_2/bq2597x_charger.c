...
#define pr_fmt(fmt)	"[bq25970-sec] %s: " fmt, __func__
...
do {											\
	if (bq->mode == BQ25970_ROLE_MASTER)						\
		printk(KERN_ERR "[bq2597x-MASTER-sec]:%s:" fmt, __func__, ##__VA_ARGS__);	\
	else if (bq->mode == BQ25970_ROLE_SLAVE)					\
		printk(KERN_ERR "[bq2597x-SLAVE-sec]:%s:" fmt, __func__, ##__VA_ARGS__);	\
	else										\
		printk(KERN_ERR "[bq2597x-STANDALONE-sec]:%s:" fmt, __func__, ##__VA_ARGS__);\
} while(0);
...
static ssize_t bq2597x_show_part_no(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct bq2597x *bq = dev_get_drvdata(dev);
	int idx = 0;
	...
	ret = bq2597x_read_byte(bq, BQ2597X_REG_13, &data);
	if (ret == 0) {
		chip_part_no = (data & BQ2597X_DEV_ID_MASK);
		chip_part_no >>= BQ2597X_DEV_ID_SHIFT;
	} else
		chip_part_no = 0xFF;
	idx = snprintf(buf, PAGE_SIZE, "%02X\n", chip_part_no);

	return idx;
}

static DEVICE_ATTR(registers, 0660, bq2597x_show_registers, bq2597x_store_register);
static DEVICE_ATTR(part_no, 0660, bq2597x_show_part_no, NULL);
...
static int bq2597x_psy_register(struct bq2597x *bq)
{
	...
	if (bq->mode == BQ25970_ROLE_MASTER)
		bq->psy_desc.name = "bq2597x-sec-master";
	else if (bq->mode == BQ25970_ROLE_SLAVE)
		bq->psy_desc.name = "bq2597x-sec-slave";
	else
		bq->psy_desc.name = "bq2597x-s-standalone";
	bq->psy_desc.type = POWER_SUPPLY_TI_TYPE_MAINS;
...
{
	if (bq->mode == BQ25970_ROLE_MASTER)
		bq->debug_root = debugfs_create_dir("bq2597x-sec-master", NULL);
	else if (bq->mode == BQ25970_ROLE_SLAVE)
		bq->debug_root = debugfs_create_dir("bq2597x-sec-slave", NULL);
	else
		bq->debug_root = debugfs_create_dir("bq2597x-s-standalone", NULL);
...
static struct of_device_id bq2597x_charger_match_table[] = {
	{
		.compatible = "ti,bq2597x-s-standalone",
		.data = &bq2597x_mode_data[BQ25970_STDALONE],
	},
	{
		.compatible = "ti,bq2597x-sec-master",
		.data = &bq2597x_mode_data[BQ25970_MASTER],
	},

	{
		.compatible = "ti,bq2597x-sec-slave",
					...
					const struct i2c_device_id *id)
{
	...
	bq_err("bq2597x sec probe successfully, Part Num:%d\n!",
				bq->part_no);
...
static void bq2597x_charger_shutdown(struct i2c_client *client)
{
	...
	pr_info("port 2 bq2597x_charger_shutdown ...");
	is_shutdown = 1;
	bq2597x_enable_adc(bq, false);
}
...
static const struct i2c_device_id bq2597x_charger_id[] = {
	{"bq2597x-s-standalone", BQ25970_ROLE_STDALONE},
	{"bq2597x-sec-master", BQ25970_ROLE_MASTER},
	{"bq2597x-sec-slave", BQ25970_ROLE_SLAVE},
	{},
};

static struct i2c_driver bq2597x_charger_driver = {
	.driver		= {
		.name	= "bq2597x-sec-charger",
		.owner	= THIS_MODULE,
