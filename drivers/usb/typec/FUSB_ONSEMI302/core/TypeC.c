...
ATOMIC_NOTIFIER_HEAD(audio_switch_notifier);
EXPORT_SYMBOL_GPL(audio_switch_notifier);
int audio_switch_reg_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&audio_switch_notifier, nb);
}
EXPORT_SYMBOL_GPL(audio_switch_reg_notifier);

void audio_switch_unreg_notifier(struct notifier_block *nb)
{
	atomic_notifier_chain_unregister(&audio_switch_notifier, nb);
}
EXPORT_SYMBOL_GPL(audio_switch_unreg_notifier);

int audio_switch_state = 0xff;
...
void SetStateUnattached(Port_t *port)
{
    ...
    pr_info("fusb %s:%d\n",__func__,__LINE__);
    if(audio_switch_state == 1){
	pr_err("jzw audio_switch_state 0");    
	audio_switch_state = 0;
    }
    atomic_notifier_call_chain(&audio_switch_notifier,
				0, port);
...
void SetStateAudioAccessory(Port_t *port)
{
    ...
    port->loopCounter = 0;
    pr_info("fusb %s:%d/n",__func__,__LINE__);
    platform_set_vbus_lvl_enable(port->PortID, VBUS_LVL_ALL, FALSE, FALSE);
