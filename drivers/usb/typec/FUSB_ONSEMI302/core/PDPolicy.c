...
void USBPDPolicyEngine(Port_t *port)
{
    ...
    case peGetPPSStatus:
        ...
        break;
    case peSinkGetStatus:
        PolicySinkGetStatus(port);
        break;
    case peSinkGetSourceCapExt:
...
void PolicySourceNegotiateCap(Port_t *port)
{
    ...
    else
    {
        ...
        notify_observers(EVENT_PD_CONTRACT_FAILED, port->I2cAddr, 0);
    }
...
#define MAX_FPDO_V 180
static int charger2_5v_obj_current = 0;
static int charger2_9v_obj_current = 0;
static int charger2_pd_max_power = 0;

int get_charger2_max_power(void) {
	return charger2_pd_max_power;
}

int get_charger2_5v_obj_current(void) {
	return charger2_5v_obj_current;
}

int get_charger2_9v_obj_current(void) {
	return charger2_9v_obj_current;
}

void reset_charger2_pd_power_data(void) {
	charger2_pd_max_power = 0;
	charger2_5v_obj_current = 0;
	charger2_9v_obj_current = 0;
}
#endif
void PolicySinkEvaluateCaps(Port_t *port)
{
...
#ifdef CONFIG_PRODUCT_MOBA
    FSC_U32 objCurrent2 = 0;
    FSC_U32 g_power = 0;
    FSC_U32 g_max_power = 0;
#endif
	...
	if (port->PortConfig.SinkRequestMaxVoltage > MAX_FPDO_V) {
		port->PortConfig.SinkRequestMaxVoltage = MAX_FPDO_V;
	}
#endif
    /* Convert from 50mV to 1mV units */
...
#ifdef CONFIG_PRODUCT_MOBA
            objCurrent2 = port->SrcCapsReceived[i].FPDOSupply.MaxCurrent * 10;
            g_power = (objVoltage * objCurrent2) / 1000000;
            pr_info("sec-usbpd-list fixed %d : volt %d, curr %d, power %d\n",
			i, objVoltage, objCurrent2, g_power);

            if ((objVoltage >= 5000) && (objVoltage <= 5300))
                charger2_5v_obj_current = objCurrent2;
            else if ((objVoltage >= 9000) && (objVoltage <= 9300))
                charger2_9v_obj_current = objCurrent2;
            if (g_power > g_max_power)
                g_max_power = g_power;
#endif
...
#ifdef CONFIG_PRODUCT_MOBA
    charger2_pd_max_power = g_max_power;
    pr_info("sec-usbpd-list max fixed power %d, 5v obj %dmA, 9v obj %dmA\n",
		charger2_pd_max_power, charger2_5v_obj_current, charger2_9v_obj_current);
#endif
            ...
            if (port->PolicyRxHeader.NumDataObjects == 0)
            {
                ...
                case CMTAccept:
                    ...
                    if (port->PpsEnabled == TRUE)
                    {
                        ...
                        port->DetachThreshold = VBUS_MV_PPS_MIN_DISC;
                    }
                    else
                    {
                        port->DetachThreshold = VBUS_MV_VSAFE5V_DISC;
                ...
                case CMTReject:
                    ...
                    notify_observers(EVENT_PD_CONTRACT_FAILED, port->I2cAddr,
                            &port->SinkRequest);
                    break;
                case CMTSoftReset:
                    SetPEState(port, peSinkSoftReset);
...
#endif /* 0 */
            case CMTGetStatus:
                SetPEState(port, peSinkGetStatus);
                break;
            case CMTGetPPSStatus:
                SetPEState(port, peGetPPSStatus);
                break;
            case CMTGetSourceCapExt:
                SetPEState(port, peSinkGetSourceCapExt);
                break;
            default:
                /* Transmit other messages directly from the GUI/DPM */
                SetPEState(port, peSendGenericCommand);
...
void UpdateCapabilitiesRx(Port_t *port, FSC_BOOL IsSourceCaps)
{
    ...
    notify_observers(EVENT_SRC_CAPS_UPDATED, port->I2cAddr, 0);
...
void PolicyGetPPSStatus(Port_t *port)
{
    ...
    default:
        if (port->ProtocolMsgRx)
        {
            ...
            {
                for(i = 0; i < 4; i++) {
                    pps_status.byte[i] = port->ExtMsgBuffer[i];
                }
                notify_observers(PPS_STATUS_RECEIVED, port->I2cAddr,
                                       &pps_status);
...
void PolicySinkGetStatus(Port_t *port)
{
    Status_t status;
    FSC_U8 i;
    ...
    {
    case 0:
        if (PolicySendCommand(port, CMTGetStatus, peSinkGetStatus, 1,
                              SOP_TYPE_SOP) == STAT_SUCCESS)
        {
            TimerStart(&port->PolicyStateTimer, tSenderResponse);
        }
    ...
    default:
        if (port->ProtocolMsgRx)
        {
            ...
            {
                for(i = 0; i < 6; i++) {
                    status.byte[i] = port->ExtMsgBuffer[i];
                }
                notify_observers(STATUS_RECEIVED, port->I2cAddr,
                                       &status);
...
void PolicySinkGetSourceCapExt(Port_t *port)
{
    ExtSrcCapBlock_t ext_src_cap;
    FSC_U8 i;

    switch(port->PolicySubIndex) {
    case 0:
        if (PolicySendCommand(port, CMTGetSourceCapExt,
                peSinkGetSourceCapExt, 1, SOP_TYPE_SOP) == STAT_SUCCESS)
        {
            TimerStart(&port->PolicyStateTimer, tSenderResponse);
        }
        break;
    default:
        if (port->ProtocolMsgRx)
        {
            ...
            if (port->PolicyRxHeader.Extended == 1 &&
                port->PolicyRxHeader.NumDataObjects > 0 &&
                port->PolicyRxHeader.MessageType == EXTSourceCapExt)
            {
                for(i = 0; i < EXT_SRC_CAP_BYTES; i++) {
                    ext_src_cap.byte[i] = port->ExtMsgBuffer[i];
                }
                notify_observers(SRC_CAPS_EXT_RECEIVED, port->I2cAddr,
                                       &ext_src_cap);
            }
            else if (port->PolicyRxHeader.Extended == 0 &&
                port->PolicyRxHeader.NumDataObjects == 0 &&
                port->PolicyRxHeader.MessageType == CMTNotSupported)
            {
                notify_observers(SRC_CAPS_EXT_RECEIVED, port->I2cAddr,
