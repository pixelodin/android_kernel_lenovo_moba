...
typedef union
{
    ...
    struct
    {
        ...
        FSC_U32 :2;
        FSC_U32 PPSPowerLimited:1;		/* PPS Power Limited */
        FSC_U32 APDOType:2;             /* (PPS: 0) */
...
typedef union
{
    ...
    struct
    {
    	...
    	FSC_U8 FixedBatt0:1;		/* Fixed Battery 0 */
    	FSC_U8 FixedBatt1:1;		/* Fixed Battery 1 */
    	FSC_U8 FixedBatt2:1;		/* Fixed Battery 2 */
    	FSC_U8 FixedBatt3:1;		/* Fixed Battery 3 */
    	FSC_U8 HotSwapBatt0:1;	/* Hot Swappable Battery 0 */
    	FSC_U8 HotSwapBatt1:1;	/* Hot Swappable Battery 1 */
    	FSC_U8 HotSwapBatt2:1;	/* Hot Swappable Battery 2 */
    	FSC_U8 HotSwapBatt3:1;	/* Hot Swappable Battery 3 */
        ...
        FSC_U8 :1;				/* Reserved */
    	FSC_U8 LimitCable:1;	/* Power limited: cable supported current */
    	FSC_U8 LimitPorts:1;	/* Power limited: insufficient power available*/
    	FSC_U8 LimitExternal:1;	/* Power limited: insufficient external power */
    	FSC_U8 LimitEvent:1;	/* Power limited: Event Flags in place */
    	FSC_U8 LimitTemp:1;		/* Power limited: temperature */
    	FSC_U8 :2;				/* Reserved */
    };
} Status_t;
...
#define EXT_SRC_CAP_BYTES 24
typedef union {
  FSC_U8 byte[EXT_SRC_CAP_BYTES] __PACKED;

  struct {
    FSC_U32 VID:16;                /* Vendor ID */
    FSC_U32 PID:16;                /* Product ID */
    FSC_U32 XID:32;                /* USB-IF extended ID */
    FSC_U32 FWVersion:8;           /* Firmware Version */
    FSC_U32 HWVersion:8;           /* Hardware Version */

	/* VoltageReg */
	FSC_U32 LoadStep:2;		/* 0: 150mA/us, 1: 500mA/us */
	FSC_U32 IoC:1;				/* 0: 25% IoC, 1: 90% IoC */
	FSC_U32 :5;				/* Reserved */

    FSC_U32 HoldupTime:8;          /* Output Holdup Time (ms) - 00: Not Supported */

	FSC_U32 LPSCompliant:1;	/* LPS Compliant when set */
	FSC_U32 PS1Compliant:1;	/* PS1 Compliant when set */
	FSC_U32 PS2Compliant:1;	/* PS2 Compliant when set */
	FSC_U32 :5;				/* Reserved */
/* Compliance */

	FSC_U32 LowTouch:1;		/* Low touch current EPS when set */
	FSC_U32 GroundPin:1;		/* Ground pin supported when set */
	FSC_U32 ProtEarth:1;		/* Ground pin intended for protective earth when set */
	FSC_U32 :5;				/* Reserved */
/* TouchCurrent */

	FSC_U32 PercOverload1:5;	/* Percent overload in 10% increments - capped at 250% */
	FSC_U32 OverloadPeriod1:6;	/* Overload period in 20ms */
	FSC_U32 DutyCycle1:4;		/* Duty cycle in 5% increments */
	FSC_U32 VoltageDroop1:1;	/* VBUS Voltage droop */
/* PeakCurrent1 */

	FSC_U32 PercOverload2:5;	/* Percent overload in 10% increments - capped at 250% */
	FSC_U32 OverloadPeriod2:6;	/* Overload period in 20ms */
	FSC_U32 DutyCycle2:4;		/* Duty cycle in 5% increments */
	FSC_U32 VoltageDroop2:1;	/* VBUS Voltage droop */
/* PeakCurrent2 */

	FSC_U32 PercOverload3:5;	/* Percent overload in 10% increments - capped at 250% */
	FSC_U32 OverloadPeriod3:6;	/* Overload period in 20ms */
	FSC_U32 DutyCycle3:4;		/* Duty cycle in 5% increments */
	FSC_U32 VoltageDroop3:1;	/* VBUS Voltage droop */
/* PeakCurrent3 */

    FSC_U32 TouchTemp:8;           /* Temperature conforms to:
     	 	 	 	 	 	 	 	 * 0: [IEC 60950-1] (default)
     	 	 	 	 	 	 	 	 * 1: [IEC 62368-1] TS1
     	 	 	 	 	 	 	 	 * 2: [IEC 62368-1] TS2
     	 	 	 	 	 	 	 	 */
	FSC_U32 ExtSupply:1;		/* External Supply Present */
	FSC_U32 Unconstrained:1;	/* External Supply is Unconstrained */
	FSC_U32 InternalBatt:1;	/* Internal Battery Present */
	FSC_U32 :5;				/* Reserved */
/* SourceInputs */

	FSC_U32 Fixed:4;			/* Number of fixed batteries */
	FSC_U32 HotSwappable:4;	/* Number of hot swappable battery slots */
/* NumBatteries */

    FSC_U32 SourcePDP:7;			/* Source's rated PDP */
    FSC_U32 :1;					/* Reserved */
  };
} ExtSrcCapBlock_t;
 ...
 */
typedef enum {
	BATTERY_STATUS_CHANGE_EVENT	= 1,
	OCP_EVENT					= 2,
	OTP_EVENT					= 3,
	OPERATING_CONDITION_CHANGE	= 4,
	SOURCE_INPUT_CHANGE_EVENT	= 5,
	OVP_EVENT					= 6,
} AlertType;
