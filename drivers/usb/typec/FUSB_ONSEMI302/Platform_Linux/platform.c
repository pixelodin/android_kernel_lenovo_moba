                                  ...
                                  FSC_BOOL disableOthers)
{
    ...
    case VBUS_LVL_5V:
        ...
        pr_info("FUSB - %s, trying to change vbus-5V, %d\n", __func__ , chip->is_vbus_present);
		if (chip->is_vbus_present != enable) {
			...
			if (enable == TRUE) {
				...
				usb2_otg_en=1;
				connect_device_type=2;
				gpio_set_value(chip->charge_2t1_gpio,1);
				gpio_set_value(chip->charge_2t2_gpio,1);
				ret =gpio_direction_output(chip->gpio_OTG, 1);
				pr_info("FUSB  %s - OTG GPIO return '%d'\n", __func__, ret);
				if (ret)
				...
				usb2_otg_en=0;
				ret = gpio_direction_output(chip->gpio_OTG, 0);
				gpio_set_value(chip->charge_2t1_gpio,0);
				gpio_set_value(chip->charge_2t2_gpio,0);
				pr_info("FUSB  %s - OTG GPIO initialized as pin '%d'\n", __func__, chip->gpio_OTG);
				if (ret)
    ...
    if ((level == VBUS_LVL_ALL) && (enable == FALSE))
    {
        // Turn off all levels
		if (chip->is_vbus_present != enable) {
			pr_info("FUSB - %s, trying to disable vbus1\n", __func__);
			//ret = regulator_disable(chip->vbus);
			usb2_otg_en=0;
			ret = gpio_direction_output(chip->gpio_OTG, 0);
			gpio_set_value(chip->charge_2t1_gpio,0);
