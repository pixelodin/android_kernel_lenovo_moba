...
#ifdef CONFIG_64BIT
    new_size = size + (sizeof(uint64_t) * 2);
#else
...
static void set_memory_prefix_and_postfix(void *buf, size_t size) {
    ...
    ((uint64_t *)buf)[0] = MEMORY_PREFIX;

    // Add the postfix to the end of the memory buffer
    *(uint64_t *)(&((uint8_t *)buf)[size - sizeof(uint64_t)]) =
            MEMORY_POSTFIX;
#else
    // Add the prefix to the beginning of the memory buffer
...
static bool is_memory_prefix_valid(void *buf) {
#ifdef CONFIG_64BIT
    uint64_t prefix = ((uint64_t *)buf)[0];

    if (prefix != MEMORY_PREFIX) {
        PRINT_CRIT("Memory corrupted, bad prefix: 0x%08llx", prefix);
        return false;
    }
#else
...
static bool is_memory_postfix_valid(void *buf, size_t size) {
#ifdef CONFIG_64BIT
    uint64_t postfix = *(uint64_t *)(&((uint8_t *)buf)[size - sizeof(uint64_t)]);

    if (postfix != MEMORY_POSTFIX) {
        PRINT_CRIT("Memory corrupted, bad posfix: 0x%08llx", postfix);
        return false;
    }
#else
